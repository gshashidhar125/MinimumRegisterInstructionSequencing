...
//==Shashi
  ...
  FileNumber++;
  if (FileNumber != 34 && sys::Program::ExecuteAndWait(prog, &args[0], 0, 0, 0, 0, &ErrMsg)) {
    errs() << "Error: " << ErrMsg << "\n";
    return;
  }
  errs() << " done. \n";
...
//==Shashi
  ...
  ostr << FileNumber; //use the string stream just like cout, except the stream prints not to stdout but to a string.
  int found = Filename.str().find_last_of("/\\");
  sys::Path CopyFilename("/home/shasg/llvm/test/DebugFiles/");
  CopyFilename.appendComponent(Filename.str().substr(found+1));
  CopyFilename.appendSuffix(ostr.str());
  CopyFilename.appendSuffix("ps");
  sys::Path copy("/bin/cp");
  std::vector<const char*> args1;
  args1.push_back(copy.c_str());
  args1.push_back(PSFilename.c_str());
  args1.push_back(CopyFilename.c_str());
  args1.push_back(0);
  errs() << "Shashidhar: Copying " << PSFilename.str() << " to " << CopyFilename.c_str() << "\n";
  if (FileNumber != 34 && sys::Program::ExecuteAndWait(copy, &args1[0],0,0,0,0,&ErrMsg)) {
    errs() << "Error: Could Not copy " << PSFilename.c_str() << " file to test/DebugFiles. " << ErrMsg << "\n";
	return;
  }
//==Shashi
